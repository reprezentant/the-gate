...
export const Hero: React.FC<HeroProps> = ({ side, name, hp, onTarget, active, lethalHighlight, maxHp = 20, mana, maxMana, avatarSrc, deckCount, handCount }) => {
  ...
  // increase max chars to 18 and give extra padding so longer names fit comfortably inside the frame
  const labelStyle: React.CSSProperties = { width: `calc(${Math.min(Math.max(labelText.length,3),18)}ch + 4rem)` };
  return (
  ...
  <div className="relative w-[4.75rem] h-[4.75rem] hero-meter order-1 mt-2 -mr-4 z-20 translate-y-2">
          <svg viewBox="0 0 100 100" className="w-full h-full overflow-visible">
            <defs>
              ...
              <linearGradient id={`mana-grad-${side}`} x1="0" y1="0" x2="0" y2="1">
                {/* Darker mana gradient: reduced white at top, deeper blue ending at #0040B7 */}
                <stop offset="0%" stopColor="#dfeefc" />
                <stop offset="50%" stopColor="#2a69a8" />
                <stop offset="100%" stopColor="#0040B7" />
              </linearGradient>
              ...
              {/* meter stroke shadow removed to avoid drop-shadow on mana meter */}
            </defs>
            {/* Decagon frame for mana */}
            <polygon points="50,0 79,10 98,35 98,65 79,91 50,100 21,91 2,65 2,35 21,10" fill="none" stroke={`url(#mana-stroke-grad-${side})`} strokeWidth="7" strokeLinejoin="round" />
            <polygon points="50,0 79,10 98,35 98,65 79,91 50,100 21,91 2,65 2,35 21,10" fill="none" stroke="#b5ecff" strokeWidth="2" strokeLinejoin="round" opacity="0.10" />
            {/* Inset content */}
              ...
              <g clipPath={`url(#mana-clip-${side})`}>
                <g style={{ transform:`translateY(${(1-manaFrac)*100}px)`, transition:'transform .75s cubic-bezier(.4,.8,.2,1)' }}>
                  <g className="mana-liquid-scroll" style={{ width:'200%', height:'100%' }}>
                    ...
                    <path className="mana-wave-front" d="M0 0 Q25 6 50 0 T100 0 T150 0 T200 0 V100 H0 Z" fill="rgba(255,255,255,0.22)" />
                    <path className="mana-wave-back" d="M0 3 Q25 8 50 3 T100 3 T150 3 T200 3 V100 H0 Z" fill="rgba(0,0,0,0.12)" />
                  </g>
                </g>
              </g>
            </g>
            <text x="50" y="44" textAnchor="middle" fontSize="24" fontWeight="700" fill="#fff" stroke="#02122a" strokeWidth="3" paintOrder="stroke" fontFamily="inherit" style={{ textShadow: 'none' }}>{mana}</text>
            <text x="50" y="69" textAnchor="middle" fontSize="14" fontWeight="700" fill="#fff" stroke="#02122a" strokeWidth="3.5" paintOrder="stroke" fontFamily="inherit" style={{ textShadow: 'none' }}>MANA</text>
          </svg>
            <div className="absolute inset-0 hero-decagon animate-meter-pulse" style={{boxShadow: '0 8px 18px rgba(0,0,0,0.18)'}} />
            {deckCount !== undefined && (
            <div className="absolute -top-11 left-[84%] -translate-x-1/2 w-[2.9rem] h-[2.9rem] hero-decagon overflow-hidden flex items-center justify-center text-white text-[10px] font-semibold tracking-wide bg-transparent translate-y-2">
              ...
              <div className="relative z-10 flex flex-col items-center leading-tight pt-[0.4rem] select-none">
                ...
                <span className="text-[12px] -mt-0.5 font-extrabold text-sky-100/90">{deckCount}</span>
              </div>
            </div>
          )}
        </div>
      )}
  <div className="relative order-2 flex flex-col items-center z-0 -mt-4 translate-y-2">
  <div className={`relative w-32 h-32 flex items-center justify-center text-white text-3xl font-black overflow-visible ${damageClass} hero-avatar`}>        
          ...
          <div className="absolute inset-0 hero-decagon overflow-hidden">
            {avatarSrc ? (
          ...
          <svg viewBox="0 0 100 100" className="absolute inset-0 w-full h-full pointer-events-none overflow-visible">
            <defs>
              ...
              {/* avatar stroke shadow intentionally removed to avoid drop-shadow on avatar */}
            </defs>
            <polygon points="50,0 79,10 98,35 98,65 79,91 50,100 21,91 2,65 2,35 21,10" fill="none" stroke={`url(#avatar-stroke-grad-${side})`} strokeWidth="10" strokeLinejoin="round" />
              <polygon points="50,0 79,10 98,35 98,65 79,91 50,100 21,91 2,65 2,35 21,10" fill="none" stroke={`url(#avatar-stroke-grad-${side})`} strokeWidth="7" strokeLinejoin="round" />
              <polygon points="50,0 79,10 98,35 98,65 79,91 50,100 21,91 2,65 2,35 21,10" fill="none" stroke="#ffb1b1" strokeWidth="2" strokeLinejoin="round" opacity="0.22" />
          </svg>
          ...
          <div className="absolute bottom-0 left-1/2 -translate-x-1/2 h-[1.65rem] flex items-center justify-center pointer-events-none translate-y-2" style={labelStyle}>
            <div className={`relative w-full h-full hero-label-frame ${active ? 'is-active' : ''}`}>
              ...
              <div className={`hero-label absolute inset-0 flex items-center justify-center text-[11px] font-bold tracking-wide uppercase text-white/90 ${active ? 'hero-label-active' : ''}`}>
                {labelText}
              </div>
            </div>
          </div>
  ...
  <div className="relative w-[4.75rem] h-[4.75rem] hero-meter order-3 mt-2 -ml-4 z-20 translate-y-2">
        <svg viewBox="0 0 100 100" className="w-full h-full overflow-visible">
            <defs>
            <clipPath id={`hp-clip-${side}`}>
              ...
              {/* Darker HP gradient: reduced white at top, deeper red ending at #BC0000 */}
              <stop offset="0%" stopColor="#ffecec" />
              <stop offset="50%" stopColor="#c53a3a" />
            ...
            <linearGradient id={`hp-stroke-grad-${side}`} x1="0" y1="0" x2="0" y2="1">
              ...
              {/* hp stroke shadow removed to avoid drop-shadow on HP meter */}
          </defs>
          {/* Decagon frame HP */}
          <polygon points="50,0 79,10 98,35 98,65 79,91 50,100 21,91 2,65 2,35 21,10" fill="none" stroke={`url(#hp-stroke-grad-${side})`} strokeWidth="7" strokeLinejoin="round" />
          <polygon points="50,0 79,10 98,35 98,65 79,91 50,100 21,91 2,65 2,35 21,10" fill="none" stroke="#ffc4c4" strokeWidth="2" strokeLinejoin="round" opacity="0.10" />
          {/* Inset content */}
          <g transform="translate(50 50) scale(.90) translate(-50 -50)">
            ...
            <g clipPath={`url(#hp-clip-${side})`}>
              <g style={{ transform:`translateY(${(1-frac)*100}px)`, transition:'transform .75s cubic-bezier(.4,.8,.2,1)' }}>
                <g className="hp-liquid-scroll" style={{ width:'200%', height:'100%' }}>
                  ...
                  <path className="hp-wave-front" d="M0 0 Q25 6 50 0 T100 0 T150 0 T200 0 V100 H0 Z" fill="rgba(255,255,255,0.24)" />
                  <path className="hp-wave-back" d="M0 3 Q25 8 50 3 T100 3 T150 3 T200 3 V100 H0 Z" fill="rgba(0,0,0,0.12)" />
                </g>
              </g>
            </g>
          </g>
          <text x="50" y="44" textAnchor="middle" fontSize="28" fontWeight="700" fill="#fff" stroke="#02122a" strokeWidth="3" paintOrder="stroke" fontFamily="inherit" style={{ textShadow: 'none' }}>{hp}</text>
          <text x="50" y="69" textAnchor="middle" fontSize="15" fontWeight="700" fill="#fff" stroke="#02122a" strokeWidth="3.5" paintOrder="stroke" fontFamily="inherit" style={{ textShadow: 'none' }}>HP</text>
        </svg>
  <div className="absolute inset-0 hero-decagon animate-meter-pulse" style={{boxShadow: '0 8px 18px rgba(0,0,0,0.18)'}} />
        {handCount !== undefined && (
          <div className="absolute -top-11 left-[16%] -translate-x-1/2 w-[2.9rem] h-[2.9rem] hero-decagon overflow-hidden flex items-center justify-center text-white text-[10px] font-semibold tracking-wide bg-transparent translate-y-2">
            <svg viewBox="0 0 100 100" className="absolute inset-0 w-full h-full pointer-events-none overflow-visible">
              <defs>
              ...
              <div className="relative z-10 flex flex-col items-center leading-tight pt-[0.4rem] select-none">
                ...
                <span className="text-[12px] -mt-0.5 font-extrabold text-sky-100/90">{handCount}</span>
              </div>
          </div>
        )}
      </div>
  ...
  /* Avatar shadows removed: keep avatar clean without drop-shadow/box-shadow */
  .hero-avatar { transition: box-shadow .25s ease; }
  .hero-avatar:hover { }
  ...
  .hero-label-frame { /* stroke visual preserved in SVG; drop-shadow removed to keep stroke-only text */ }
