...
function HeroPowerFloating({ activeTurn, used, mana, cost, onUse }: { activeTurn: boolean; used: boolean; mana: number; cost: number; onUse: () => void; }) {
      ...
      <div className="fixed bottom-6 right-6 z-[60] pointer-events-auto group">
        ...
        >
          ...
          <div className="hpwr-particles absolute inset-0 pointer-events-none overflow-visible">
            {particles.map(p => {
              const style = { ['--tx' as string]: p.tx + 'px', ['--ty' as string]: p.ty + 'px', ['--rot' as string]: p.rot + 'deg' } as React.CSSProperties;
...
export default function CardGameMVP() {
  ...
  async function executeAiQueue(queue: AiAction[]) {
    for (const action of queue) {
      ...
      if (action.kind==='ATTACK_MINION') {
        const current = gsRef.current; const atk = current.ai.board.find(m=> m.entityId===action.attackerId); const def = current.player.board.find(m=> m.entityId===action.targetId); if (!atk || !def) continue; const aEl = document.querySelector(`[data-minion='${atk.entityId}']`) as HTMLElement | null; const dEl = document.querySelector(`[data-minion='${def.entityId}']`) as HTMLElement | null; if (!aEl || !dEl) continue; const a=aEl.getBoundingClientRect(), d=dEl.getBoundingClientRect(); setAttackAnim({ owner:'AI', attackerId: atk.entityId, targetType:'MINION', targetSide:'PLAYER', targetMinionId:def.entityId, start:{x:a.left+a.width/2,y:a.top+a.height/2}, end:{x:d.left+d.width/2,y:d.top+d.height/2} }); await wait(420); setGs(prev => { const next = clone(prev); const A = next.ai.board.find(m=> m.entityId===action.attackerId); const D = next.player.board.find(m=> m.entityId===action.targetId); if (A && D) { const db = D.currentHealth; const ab = A.currentHealth; D.currentHealth -= A.baseAttack; A.currentHealth -= (CARDS[D.cardId] as MinionCard).attack; spawnDamage(`[data-minion='${D.entityId}']`, db - Math.max(D.currentHealth,0)); spawnDamage(`[data-minion='${A.entityId}']`, ab - Math.max(A.currentHealth,0)); if (A.currentHealth>0) A.canAttack=false; next.log.push(`AI: atakuje ${CARDS[D.cardId].name}`); processDeaths(next); } return applyWinner(next); }); setAttackAnim(null); await wait(140); continue;
      }
  ...
  if (gs.winner) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-900 text-white">
        ...
        <button onClick={() => { setAttackerId(null); setGs(initGame()); }} className="px-4 py-2 bg-yellow-400 text-black rounded-lg shadow">Play Again</button>
      </div>
    );
  }
  ...
  const Hand = () => (
    <HandComp
      ...
      onPlayMinion={(i) => setGs(playCardForSidePure(gs, 'PLAYER', i))}
  ...
  const arenaBgStyle: React.CSSProperties = {
  ...
  const arenaScaleStyle: React.CSSProperties & Record<string,string> = {
  ...
  {(attackerId || pendingSpell !== null) && (
        ...
        {/* Mulligan overlay */}
        {!gs.mulliganDone && (
          <div className="absolute inset-0 z-50 flex flex-col items-center justify-center backdrop-blur-sm bg-black/60 text-white">
            <div className="text-xl font-bold mb-4">MULLIGAN – wybierz karty do wymiany (kliknij)</div>
            <div className="text-sm mb-3 opacity-80 max-w-md text-center">Wymienione karty wrócą do talii i zostaną dociągnięte nowe. Gwarancja: co najmniej jedna karta o koszcie ≤1 pozostanie w ręce.</div>
            <div className="w-full max-w-4xl px-6">
              <div className="mb-4">
                {/* Ręka do mulliganu */}
                <div className="relative rounded-xl border border-white/20 bg-black/20 p-4 backdrop-blur-sm">
                  <Hand />
                </div>
              </div>
              <div className="mb-4 flex flex-wrap gap-2 text-[13px] font-mono items-center">
                <div className="px-3 py-1 rounded-full bg-white/10 border border-white/20">Zaznaczone: {mulliganMarked.size}</div>
                <button onClick={()=> setMulliganMarked(new Set())} className="px-3 py-1 rounded-full bg-white/10 hover:bg-white/20 border border-white/20">Reset</button>
                <button
                  onClick={()=> { setGs(g => performMulligan(g, Array.from(mulliganMarked))); setMulliganMarked(new Set()); }}
                  className="ml-auto px-6 py-2 rounded-full bg-emerald-400 hover:bg-emerald-300 text-black font-extrabold shadow-lg text-sm tracking-wide disabled:opacity-40 disabled:cursor-not-allowed"
                >Gotowe</button>
              </div>
            </div>
            <div className="absolute bottom-4 text-[12px] opacity-60">Mulligan – etap początkowy</div>
          </div>
        )}
...
function ArrowOverlay({ origin, pointer, containerRef, color }: ArrowOverlayProps) {
  ...
  return (
    // Lower z-index so HUD / panels remain visible while aiming
    <svg className="pointer-events-none absolute inset-0 overflow-visible z-20">
